...
type AdminFooterNewsHttpMethod = "GET" | "POST" | "PUT" | "DELETE";
type AdminFooterNewsEndpoint = `/api/admin/footer-news${string}`;
...
async function buildForwardPayload(
  request: Request,
  method: AdminFooterNewsHttpMethod,
...
export async function forwardAdminFooterNewsRequest(
  request: Request,
  endpoint: AdminFooterNewsEndpoint,
  method: AdminFooterNewsHttpMethod,
  ...
  return NextResponse.json(
    { message: "Footer news admin service is unavailable." },
    { status: 502 },
  );
}
